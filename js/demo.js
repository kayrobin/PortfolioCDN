 ...
 */
{
    ...
    class GridItem {
        constructor(el) {
			...
			// The following DOM elements are elements that will move when hovering the item.
			this.DOM.scale = {};
			// The image.
			this.DOM.imgWrap = this.DOM.el.querySelector('.grid__item-wrap');
			this.DOM.scale.img = this.DOM.imgWrap.querySelector('img');
			// The title (vertical text).
			this.DOM.scale.title = this.DOM.el.querySelector('.grid__item-title');
		}
		/**
		 * Hides the item:
		 * - Scales down and fades out the image and bg elements.
		 * - Moves down and fades out the title element.
		 */
		...
		toggle(withAnimation, show = true) {
			gsap.to(this.DOM.scale.img, withAnimation ? 0.8 : 0, {
				ease: "expo.inOut",
				delay: !withAnimation ? 0 : show ? 0.15 : 0,
				scale: show ? 1 : 0,
				opacity: show ? 1 : 0,
			});
			gsap.to(this.DOM.bg, withAnimation ? 0.8 : 0, {
				ease: "expo.inOut",
		...
		toggleTexts(delay, withAnimation, show = true) {
			gsap.to([this.DOM.scale.title], !withAnimation ? 0 : show ? 1 : 0.5, {
				ease: show ? "expo.out" : "power1.in",
    ...
    class Content {
		...
		/**
		 * Show/Hide the content elements (title, the subtitle and the text).
		 */
		...
		toggle(delay, withAnimation, show = true) {
			setTimeout(() => {
				gsap.to(this.DOM.title, !withAnimation ? 0 : show ? 1 : 0.8, {
    ...
    class Grid {
		...
		openItem(item) {
			...
			gsap.to(item.DOM.scale.img, 1.2, {
				ease: "expo.inOut",
				delay: 0.55,
				...
				onComplete: () => {
					...
					item.DOM.scale.img.style.opacity = 0;
		...
		closeItem(withAnimation = true) {
			...
			gsap.to(item.DOM.bg, withAnimation ? 1.2 : 0, {
				ease: "expo.inOut",
				delay: 0.15,
				x: 0,
				y: 0,
		...
		toggleContentElems(contentEl, delay, withAnimation, show = true) {
			...
			gsap.to([this.DOM.closeCtrl, this.DOM.scrollIndicator], withAnimation ? 0.8 : 0, {
				ease: show ? "expo.in" : "expo.out",
				delay: withAnimation ? delay : 0,
